...
# -*- coding: utf-8 -*-
"""
CatBoost model noise analysis and uncertainty analysis
...
Note: This script needs to run CatBoost_train.py first to train the model, then load the saved model for analysis
"""
...
def configure_plot_fonts(fonts=None):
    """Set Matplotlib fonts, ensure negative sign is displayed correctly"""
    if fonts is None:
...
def find_project_root():
    ...
    # Try to locate from script position
    try:
...
def load_data():
    ...
    if 'w/c' in feature_names:
        ...
        if np.max(wc_values) - np.min(wc_values) < 1e-6:
            print(f"  ⚠ Warning: w/c feature has near-zero range in original data!")
...
def split_data_by_divisions(X, y, sample_divisions, sample_ids, test_ratio=0.2, val_ratio=0.2, random_state=42):
    ...
    for i, division in enumerate(sample_divisions):
        ...
        elif division == 'train' or division == 'training set' or division_str.startswith('train'):
            train_indices.append(i)
    ...
    # If there is no validation set but there is training set, split validation set from training set
...
def plot_shap_analysis(model, X_background, X_explain, feature_names, save_dir):
    ...
    Traditional explanation method description:
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print("✓ Successfully used KernelExplainer (note: calculation speed is slow)")
            except Exception as e3:
                print(f"⚠ All explainer initialization methods failed: {e3}")
                raise ValueError(f"Unable to initialize SHAP explainer: {e3}")
    
    # Calculate SHAP values (for explanation data set)
    print("Calculating SHAP values...")
    shap_values = explainer.shap_values(X_explain)
    
    # Set font to avoid Chinese乱码，并确保负号正常显示
    ...
    # 1. Summary plot
    # Increase width significantly, truly stretch the horizontal axis ratio
    ...
    # Get current axes
    ax = plt.gca()
    # Do not change data range, only increase graph width to stretch the horizontal axis physical ratio
    plt.tight_layout()
    ...
    for sample_idx in range(n_samples_waterfall):
        ...
        ax = plt.gca()
        # Force display negative sign and font
    ...
    # Create an explanation for force plot containing multiple samples
    ...
    # 6. Analyze the reasons why the features affect small (print detailed analysis)
    print("\n" + "="*60)
    ...
    for idx, row in shap_importance.tail(5).iterrows():
        print(f"  {row['feature']:10s}: 平均影响 = {row['shap_importance']:.4f}, "
              f"Average SHAP value = {row['shap_mean']:+.4f}, Standard deviation = {row['shap_std']:.4f}")
    
    # Analyze the reasons why the features affect small
    ...
    if len(low_importance_features) > 0:
        ...
        print("  4. This feature has a small impact on the target variable,which is consistent with physical/engineering principles.")
...
def analyze_feature_statistics(X, y, feature_names, shap_importance, save_dir):
    ...
    for idx, row in shap_importance.iterrows():
        ...
        if feat_name == 'w/c':
            ...
            if feat_range < 1e-6:
                ...
                print(f"    1. All values in the w/c column are the same")
    ...
    # Analyze the features that affect small
    ...
    if len(low_importance) > 0:
        ...
        for idx, row in low_importance.iterrows():
            ...
            if row['cv'] < 0.1:
                reasons.append("Small CV (small data change range)")
    ...
    # Analyze the features that affect large
    ...
    # 3. Feature value range (sorted by SHAP importance, consistent with SHAP summary plot)
    ax3 = axes[1, 0]
    # feature_stats is already sorted by SHAP importance in descending order, take the first 10
    # Note: SHAP summary plot is from top to bottom, so the most important features are at the top
    ...
    if len(wc_feature) > 0:
        ...
        if wc_row['range'] < 1e-6:
            ...
            print(f"  This will cause SHAP importance to be 0, because when the feature value is unchanged, it cannot affect the prediction.")
    ...
    # But keep them in top_features for user to see the problem
    ...
    ax3.set_title('Feature value range of the 10 most important features (sorted by SHAP importance)', fontsize=14, fontweight='bold')
    ...
    ax4.set_title('Correlation between features and target variable (absolute value top 10)', fontsize=14, fontweight='bold')
...
def plot_pdp_analysis(model, X_train, feature_names, save_dir, n_top_features=5):
    ...
    n_rows = (n_top_features + 2) // 3  # Calculate the number of rows
    ...
    for idx, (feature, ax) in enumerate(zip(top_features, axes)):
        try:
            ...
            ice_upper_ci = ice_mean + 1.96 * ice_std  # 95% CI (confidence interval)
    ...
    print(f"Single variable PDP analysis completed, the dependence plot of the {n_top_features} most important features has been drawn")
    ...
    if n_top_features >= 2:
        ...
        n_cols = 3  # Change to 3 columns, more beautiful
        n_rows = (n_pairs + 2) // 3  # Calculate the number of rows
        ...
        for idx, (feat1, feat2) in enumerate(interaction_pairs):
            try:
                ...
                # Get the grid values and average prediction values
        ...
        print(f"Double variable PDP analysis completed, the interaction plot of the {n_interaction_features} most important features has been drawn")
...
def analyze_noise_robustness(model, X_test, y_test, feature_names, save_dir):
    ...
    outlier_severity = 5.0  # Outlier严重程度（标准差的倍数）
    outlier_rmses = []
    outlier_stds = []
    ...
    # 3. Plot robustness analysis plot
    ...
    print(f"   - 2% noise RMSE: {noise_rmses[1]:.3f} MPa (增加 {noise_rmses[1]-noise_rmses[0]:.3f} MPa)")
    print(f"   - 4% noise RMSE: {noise_rmses[2]:.3f} MPa (增加 {noise_rmses[2]-noise_rmses[0]:.3f} MPa)")
    print(f"   - 6% noise RMSE: {noise_rmses[3]:.3f} MPa (增加 {noise_rmses[3]-noise_rmses[0]:.3f} MPa)")
    print(f"   - 8% noise RMSE: {noise_rmses[4]:.3f} MPa (增加 {noise_rmses[4]-noise_rmses[0]:.3f} MPa)")
    print(f"   - 10% noise RMSE: {noise_rmses[5]:.3f} MPa (增加 {noise_rmses[5]-noise_rmses[0]:.3f} MPa)")
    ...
    print(f"   - Outlier robustness: {outlier_robust} (10% outlier performance change: {outlier_degradation:.1f}%)")
...
def analyze_prediction_intervals(model, X_test, y_test, save_dir, n_bootstrap=100, bootstrap_noise_level=0.01):
    ...
    for _ in range(n_bootstrap):
        ...
        # Add small amount of noise to the test data to reflect the response of the model to input uncertainty
    ...
    # Build a prediction interval using the standard deviation of the residuals
    ...
    # 3. Draw a comparison plot of prediction intervals
    fig = plt.figure(figsize=(20, 10))
    
    # Create a grid layout: left two columns display prediction intervals, right column displays performance comparison
...
def load_trained_model(model_path):
    """Load a trained CatBoost model (supports .joblib and .pkl formats)"""
    ...
    if feature_names:
        ...
        print("⚠ Warning: No feature names found, will use the feature names when loading the data")
    
    return model, model_data

def cross_subset_validation(X, y, feature_names, sample_divisions, sample_ids, original_df_indices, df_processed, save_dir=None, n_trials=100):
    """subset1/2/3 three-fold cross-validation"""
    print("\n=== CatBoost Subset three-fold cross-validation ===")
    ...
    print(f"\n{'='*60}")
    print(f"Best hyperparameters in three-fold cross-validation (from fold {best_fold_idx+1}, validation set={best_fold_result['val_label']})")
    ...
    print(f"\n{'='*60}")
    print("Use the best fold model as the final model (no re-training)")
    print(f"{'='*60}")
    ...
    # Evaluate on the test set (should be consistent with the fold metrics)
    ...
    try:
        ...
        if 'No_Customized' in df_original.columns:
            df_original['CatBoost_fc'] = df_original['No_Customized'].map(pred_dict)
        else:
            print("Warning: No_Customized column not found, cannot match prediction results")
        
        # Save the results
        output_file = os.path.join(PROJECT_ROOT, "dataset/dataset_with_CatBoost_fc.xlsx")
        df_original.to_excel(output_file, index=False)
        print(f"✓ Prediction results written to: {output_file}")
        print(f"  - New column: CatBoost_fc (final model predicted values)")
        print(f"  - Number of non-empty predicted values: {df_original['CatBoost_fc'].notna().sum()}")
        
        # Backup
        backup_file = os.path.join(save_dir, 'dataset_with_CatBoost_fc_backup.xlsx')
...
def cross_random_kfold_validation(X, y, feature_names, sample_ids, sample_divisions=None,
    ...
    print(f"\n=== CatBoost Random {n_splits} fold cross-validation ===")
    ...
    try:
        ...
        if 'No_Customized' in df_original.columns:
            df_original['CatBoost_fc_random'] = df_original['No_Customized'].map(pred_dict)
        else:
            df_original['CatBoost_fc_random'] = y_all_pred

        output_file = os.path.join(save_dir, "dataset_with_CatBoost_fc_random.xlsx")
        df_original.to_excel(output_file, index=False)
        print(f"✓ Random split model prediction results written to: {output_file}")
        print(f"  - New column: CatBoost_fc_random")
        print(f"  - Number of non-empty predicted values: {df_original['CatBoost_fc_random'].notna().sum()}")
    except Exception as e:
...
def main():
    ...
    print("=== CatBoost SHAP analysis, noise analysis and uncertainty analysis ===")
    ...
    # 2. Load the trained model - directly use catboost_final_model.joblib
    model_path = os.path.join(SAVE_ROOT, 'catboost_final_model.joblib')
    
    if not os.path.exists(model_path):
        print(f"Error: Model file not found: {model_path}")
        print("Ensure the model file exists in: peak_stress/CatBoost/save/catboost_final_model.joblib")
        return
    ...
    # Use all data for SHAP analysis (dataset is small, total 94 samples)
