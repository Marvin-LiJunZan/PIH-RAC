...
# -*- coding: utf-8 -*-
"""
RandomForest model noise analysis and uncertainty analysis
...
Note: This script needs to run RandomForest_train.py to train the model first, then load the saved model for analysis
"""
...
def configure_plot_fonts(fonts=None):
    """Set matplotlib fonts uniformly, ensure negative sign is displayed correctly"""
    if fonts is None:
...
plt.rcParams['mathtext.default'] = 'regular'  # Use regular fonts to render mathematical symbols

# Set working directory - automatically find project root directory
def find_project_root():
    """Find project root directory"""
    # Try to locate the script position first
    try:
...
def load_data():
    ...
    # Save the original DataFrame indices (for subsequent mapping prediction results)
...
def split_data_by_divisions(X, y, sample_divisions, sample_ids, test_ratio=0.2, val_ratio=0.2, random_state=42):
    ...
    for i, division in enumerate(sample_divisions):
        ...
        if division_str.lower().startswith('test'):
            ...
            # subset1, subset2, subset3 are all used as training sets (or can be allocated as needed)
            # Here all are used as training sets by default, if validation sets are needed, they can be split from the subsets
    ...
    # If there is no validation set but there is a training set, split the validation set from the training set
    # If the test set exists, it remains unchanged
    if train_indices and not val_indices:
        print("Only training set samples, splitting validation set from the training set...")
        from sklearn.model_selection import train_test_split
        
        train_indices = np.array(train_indices)
        
        if len(train_indices) > 1:
            # If the test set exists, only split the validation set from the training set
            # If the test set does not exist, split the validation set and test set from the training set
            if len(test_indices) > 0:
                # Test set already exists, only split the validation set
                ...
                # Test set does not exist, split the validation set and test set from the training set
...
def objective(trial, X_train, y_train, X_val, y_val):
    """Optuna optimization objective function - using validation set for evaluation"""
    ...
    # Use internal cross-validation on the training set for evaluation, more stable
...
def optimize_randomforest(X_train, y_train, X_val, y_val, n_trials=100):
    """Use Optuna to optimize RandomForest hyperparameters - using 5-fold cross-validation for evaluation"""
    print(f"\n=== Start RandomForest hyperparameter optimization (n_trials={n_trials}) ===")
    ...
    try:
        save_dir = os.path.join(SAVE_ROOT, 'randomforest_optimization')
...
def train_randomforest(X_train, y_train, X_val, y_val, best_params):
    """Use best parameters to train RandomForest model"""
...
def plot_results(y_true, y_pred, save_dir, model_name='randomforest'):
    """Plot results"""
    print("\n=== Plot results ===")
    ...
    fig.suptitle(f'RandomForest Results ($R^2$={r2:.4f})', fontsize=16, fontweight='bold')
...
- Feature Importance Analysis"""
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'randomforest_results.png'), dpi=300, bbox_inches='tight')
    plt.close()
    
    return r2, mae, rmse

def plot_train_test_comparison(model, X_train, y_train, X_test, y_test, save_dir):
    """Plot the comparison of training set and test set (with edge distribution) - optimized version
    
    Returns:
        DataFrame: DataFrame containing the prediction results of the training set and test set
    """
    print("\n=== Plot the comparison of training set and test set ===")
    ...
    # Training set - using more professional blue
    ...
    # Calculate and plot the best fit line and confidence interval (training set)
    ...
    # Optimized legend - placed in the lower right corner, using LaTeX format to display R²
    legend_elements = [
    ...
    # Create DataFrame containing the prediction results of the training set and test set
...
def plot_feature_importance(model, feature_names, save_dir, X=None):
    ...
    # Save to Excel (will be saved to the comprehensive Excel file in the main function)
    # Here we don't save directly, return DataFrame for main function to save
...
def plot_shap_analysis(model, X_background, X_explain, feature_names, save_dir):
    ...
    # RandomForest model can directly use TreeExplainer
    ...
    try:
        ...
        try:
            # Don't pass in background data set, use default value
    ...
    # Set font, avoid Chinese characters, and ensure negative sign is displayed correctly
    original_font = list(plt.rcParams['font.sans-serif'])
    original_unicode_minus = plt.rcParams['axes.unicode_minus']
    
    # For SHAP plots, use DejaVu Sans to ensure negative sign is displayed correctly
    ...
    # Get current axes
    ax = plt.gca()
    # Don't change data range, only increase graph width to stretch the horizontal axis physical ratio
    plt.tight_layout()
    ...
    # Plot force plot for the first 3 samples
    ...
    # 6. Analyze the reasons why some features have a small impact (print detailed analysis)
    print("\n" + "="*60)
    ...
    # Analyze the possible reasons why some features have a small impact
    ...
    if len(low_importance_features) > 0:
        ...
        print("\nPossible reasons:")
        print("  1. The range of feature values is small (small variance), resulting in limited impact on prediction")
        print("  2. Highly correlated with other features, replaced by other features")
        print("  3. Feature values are evenly distributed in the data set, lack of distinction")
        print("  4. This feature has a small impact on the target variable with physical/engineering principles")
...
def analyze_feature_statistics(X, y, feature_names, shap_importance, save_dir):
    """Analyze feature statistical information, explain why some features have a small impact"""
    print("\n" + "="*60)
    print("Feature statistical analysis (explain why some features have a small impact)")
    print("="*60)
    ...
    # Rearrange the column order to make it more logical: basic statistics -> quartiles -> derived statistics -> importance
    ...
    # Save statistical information (will be saved in the main function)
    # Return DataFrame for main function to save
    ...
    if len(low_importance) > 0:
        print(f"\nThere are {len(low_importance)} features with a small impact:")
    ...
    # Plot feature statistical graph
    ...
    if len(wc_feature) > 0:
        ...
        print(f"\n⚠ Warning: w/c feature statistical information:")
        ...
        if wc_row['range'] < 1e-6:
            print(f"  ⚠ Warning: w/c feature value range is close to 0, all values may be the same!")
            ...
            print(f"  Please check the actual values in the w/c column in the data.")
    
    # Filter out features with range 0 or close to 0 (these features should not appear in the importance plot)
    # But keep in top_features for users to see the problems
    ...
    if len(valid_range_features) < len(top_features):
        ...
        print(f"  All features will be plotted (including features with range 0) to find problems")
    
    y_pos = np.arange(len(top_features))
    # Plot horizontal bar chart, the most important feature at the top (index 0)
    # For features with range 0, use a small value (such as 1e-6) to display on the graph
    bar_values = top_features['range'].values.copy()
    bar_values[bar_values < 1e-6] = 1e-6  # Replace 0 values with a small value to display
    ...
    ax3.set_title('Value range of the top 10 important features (sorted by SHAP importance)', fontsize=14, fontweight='bold')
    ax3.grid(True, alpha=0.3, axis='x')
    # Invert y axis, so the most important feature is at the top (consistent with SHAP summary plot: importance decreases from top to bottom)
    ax3.invert_yaxis()
    ...
    print("\n✓ Feature statistical analysis completed, results saved")
    print("="*60)
...
def plot_pdp_analysis(model, X_train, feature_names, save_dir, n_top_features=5):
    ...
    # plt.show()  # Show univariate PDP plots
    plt.close()
    
    print(f"Univariate PDP analysis completed, plots for the top {n_top_features} important features")
    ...
    if n_top_features >= 2:
        ...
        for idx, (feat1, feat2) in enumerate(interaction_pairs):
            try:
                ...
                # For 3D graph, check if it is a subplot
                if idx < len(axes):
        ...
        print(f"Bivariate PDP analysis completed, plots for the interaction between the top {n_interaction_features} important features")
...
def analyze_noise_robustness(model, X_test, y_test, feature_names, save_dir):
    ...
    print(f"   - 0% noise when RMSE: {noise_rmses[0]:.3f} MPa")
    print(f"   - 2% noise when RMSE: {noise_rmses[1]:.3f} MPa (increase {noise_rmses[1]-noise_rmses[0]:.3f} MPa)")
    print(f"   - 4% noise when RMSE: {noise_rmses[2]:.3f} MPa (increase {noise_rmses[2]-noise_rmses[0]:.3f} MPa)")
    print(f"   - 6% noise when RMSE: {noise_rmses[3]:.3f} MPa (increase {noise_rmses[3]-noise_rmses[0]:.3f} MPa)")
    print(f"   - 8% noise when RMSE: {noise_rmses[4]:.3f} MPa (increase {noise_rmses[4]-noise_rmses[0]:.3f} MPa)")
    print(f"   - 10% noise when RMSE: {noise_rmses[5]:.3f} MPa (increase {noise_rmses[5]-noise_rmses[0]:.3f} MPa)")
    
    print("\n2. Outliers robustness:")
    print(f"   - 0% outliers when RMSE: {outlier_rmses[0]:.3f} MPa")
    print(f"   - 2% outliers when RMSE: {outlier_rmses[1]:.3f} MPa (change {outlier_rmses[1]-outlier_rmses[0]:.3f} MPa)")
    print(f"   - 4% outliers when RMSE: {outlier_rmses[2]:.3f} MPa (change {outlier_rmses[2]-outlier_rmses[0]:.3f} MPa)")
    print(f"   - 6% outliers when RMSE: {outlier_rmses[3]:.3f} MPa (change {outlier_rmses[3]-outlier_rmses[0]:.3f} MPa)")
    print(f"   - 8% outliers when RMSE: {outlier_rmses[4]:.3f} MPa (change {outlier_rmses[4]-outlier_rmses[0]:.3f} MPa)")
    print(f"   - 10% outliers when RMSE: {outlier_rmses[5]:.3f} MPa (change {outlier_rmses[5]-outlier_rmses[0]:.3f} MPa)")
    ...
    print(f"   - Robustness to Gaussian noise: {noise_robust} (performance degradation {noise_degradation:.1f}% when 10% noise)")
    print(f"   - Robustness to outliers: {outlier_robust} (performance change {outlier_degradation:.1f}% when 10% outliers)")
...
def analyze_prediction_intervals(model, X_test, y_test, save_dir, n_bootstrap=100, bootstrap_noise_level=0.01):
    """Analyze prediction interval confidence
    ...
    Method description:
    1. Bootstrap: Simulate model uncertainty by adding input noise, similar to Monte Carlo Dropout
    2. Quantile Regression: Build prediction intervals with fixed width based on residual distribution
    """
    ...
    for _ in range(n_bootstrap):
        ...
        # Add small amount of noise to the test data, reflecting the model's response to input uncertainty
    ...
    # Save in Excel in the main function
...
def train_single_fold(X_train, y_train, X_val, y_val, X_test, y_test, train_ids, val_ids, test_ids, 
    ...
    study = optimize_randomforest(X_train, y_train, X_val, y_val, n_trials=n_trials)
    best_params = study.best_params
    
    # Train final model
    model = train_randomforest(X_train, y_train, X_val, y_val, best_params)
...
def load_trained_model(model_path):
    """Load trained RandomForest model (supports .joblib and .pkl formats)"""
    print(f"\n=== Load trained model ===")
    ...
    if feature_names:
        ...
        print("⚠ Warning: No feature name information found, using feature names from data loading")
...
def cross_subset_validation(X, y, feature_names, sample_divisions, sample_ids, original_df_indices, df_processed, 
    ...
    print("\n=== RandomForest Subset three-fold cross-validation ===")

    if save_dir is None:
        save_dir = os.path.join(SAVE_ROOT, 'randomforest_cv')
    ...
    print(f"\n{'='*60}")
    print(f"Best hyperparameters in three-fold cross-validation (from round {best_fold_idx+1}, validation set={best_fold_result['val_label']})")
    ...
    print(f"\n{'='*60}")
    print("Use best fold model as final model (no retraining)")
    print(f"{'='*60}")
    ...
    # Evaluate on test set (should match the metrics of the best fold)
    ...
    }, os.path.join(interpretability_dir, 'final_randomforest_model.pkl'))
    ...
    try:
        ...
        if 'No_Customized' in df_original.columns:
            df_original['RandomForest_fc'] = df_original['No_Customized'].map(pred_dict)
        else:
            print("Warning: No_Customized column not found, cannot match prediction results")
        
        # Save results
        output_file = os.path.join(PROJECT_ROOT, "dataset/dataset_with_RandomForest_fc.xlsx")
        df_original.to_excel(output_file, index=False)
        print(f"✓ Prediction results written to: {output_file}")
        print(f"  - New column: RandomForest_fc (final model predicted values)")
        print(f"  - Number of non-empty prediction values: {df_original['RandomForest_fc'].notna().sum()}")
        
        # Backup
        backup_file = os.path.join(save_dir, 'dataset_with_RandomForest_fc_backup.xlsx')
...
def cross_random_kfold_validation(X, y, feature_names, sample_ids, sample_divisions=None,
    ...
    print(f"\n=== RandomForest Random {n_splits} fold cross-validation ===")

    if save_dir is None:
        save_dir = os.path.join(SAVE_ROOT, 'randomforest_cv_random')
    ...
    if fixed_test_indices is not None:
        ...
        print(f"Random cross-validation only on non-test set samples, {len(train_pool_indices)} samples")
    ...
    joblib.dump({
        ...
        'shap_importance': shap_importance,
        'cross_validation_results': all_results
    }, os.path.join(interpretability_dir, 'final_randomforest_model_random.pkl'))
    ...
    try:
        ...
        if 'No_Customized' in df_original.columns:
            df_original['RandomForest_fc_random'] = df_original['No_Customized'].map(pred_dict)
        else:
            df_original['RandomForest_fc_random'] = y_all_pred

        output_file = os.path.join(save_dir, "dataset_with_RandomForest_fc_random.xlsx")
        df_original.to_excel(output_file, index=False)
        print(f"✓ Random divided model prediction results written to: {output_file}")
        print(f"  - New column: RandomForest_fc_random")
        print(f"  - Number of non-empty prediction values: {df_original['RandomForest_fc_random'].notna().sum()}")
    except Exception as e:
...
def main():
    ...
    print("=== RandomForest SHAP analysis, noise analysis and uncertainty analysis ===")
    ...
    # 2. Load trained model - directly use randomforest_final_model.joblib
    model_path = os.path.join(SAVE_ROOT, 'randomforest_final_model.joblib')
    
    if not os.path.exists(model_path):
        print(f"Error: Model file not found: {model_path}")
        print("Please ensure the model file exists in: peak_stress/RandomForest/save/randomforest_final_model.joblib")
        return
    ...
    # 11. Save all data to an Excel file in different sheets
    print(f"\n{'='*60}")
