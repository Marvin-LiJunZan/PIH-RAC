...
# -*- coding: utf-8 -*-
"""
XGBoost model noise analysis and uncertainty analysis - peak strain prediction
...
Note: This script requires running XGBoost_train.py to train the model first, then load the saved model for analysis
This script is used for peak strain (peak_strain) prediction, using 17 features (15 material parameters + fc + Xiao_strain)
"""
...
def configure_plot_fonts(fonts=None):
    """Set Matplotlib fonts uniformly, ensuring negative signs display correctly"""
    if fonts is None:
...
def find_project_root():
    ...
    try:
        ...
        # Running in Jupyter, search upwards from current directory
        pass
...
def load_data():
    """Load data - use 15 material parameters + peak stress (fc) + Xiao_strain to predict peak strain"""
    print("=== Load data ===")
    ...
    if missing_cols:
        ...
        if "Xiao_strain" in missing_cols and "fc" in df.columns and "r" in df.columns:
            print("  Trying to compute Xiao_strain...")
            df["Xiao_strain"] = compute_xiao_strain(df["fc"].values, df["r"].values)
            print("  ✓ Xiao_strain computed and added")
...
def split_data_by_divisions(X, y, sample_divisions, sample_ids, test_ratio=0.2, val_ratio=0.2, random_state=42):
    ...
    for i, division in enumerate(sample_divisions):
        ...
        if division_str.lower().startswith('test'):
            ...
            # subset1, subset2, subset3 are all used as training set (or can be allocated as needed)
...
def objective(trial, X_train, y_train, X_val, y_val):
    ...
    params = {
        ...
        # Hyperparameter search space (XGBoost parameters)
    ...
    # Use internal cross-validation of training set for evaluation, more stable
...
def optimize_xgboost(X_train, y_train, X_val, y_val, n_trials=100):
    ...
    print(f"\n=== 开始XGBoost超参数优化 (n_trials={n_trials}) ===")
...
def plot_train_test_comparison(model, X_train, y_train, X_test, y_test, save_dir):
    """Plot training set and test set comparison plot (with marginal distribution) - optimized version
    ...
    # Optimized legend - placed in lower right corner, use LaTeX format to display R²
    legend_elements = [
    ...
    # Add total title - contains sample count information
...
# CatBoost model does not need to be repaired, directly use it
...
def plot_shap_analysis(model, X_background, X_explain, feature_names, save_dir, y_explain=None):
    ...
    Traditional做法说明:
        ...
        - Explanation dataset (X_explain): using test set
          Explain the unseen data, evaluate the explainability of the model's generalization ability
        
        Advantages:
        1. Conforms to machine learning best practices (training/test separation)
        2. Evaluates the explainability of the model in real application scenarios
        3. Avoids data leakage (not using test set to train explainer)
    """
    print("\n=== Perform SHAP analysis ===")
    print(f"Background dataset sample count: {len(X_background)} (used to initialize explainer)")
    print(f"Explanation dataset sample count: {len(X_explain)} (used to calculate SHAP values)")
    
    # XGBoost model can directly use TreeExplainer
    # If the background dataset is passed in error, it can be not passed in background dataset (using default value)
    try:
        # Try to use background dataset to initialize explainer
        explainer = shap.TreeExplainer(model, X_background)
        print("✓ Successfully initialized explainer using background dataset")
    except (ValueError, TypeError, AttributeError) as e:
        print(f"⚠ Failed to initialize explainer using background dataset: {e}")
        print("   Try to initialize explainer without background dataset...")
        try:
            # Do not pass in background dataset, use default value
            explainer = shap.TreeExplainer(model)
            print("✓ Successfully initialized explainer without background dataset")
            ...
            try:
                ...
                print("✓ Successfully initialized KernelExplainer (note: calculation is slower)")
    ...
    # Set font to avoid Chinese characters, and ensure negative sign is displayed correctly
    ...
    # Get current axes
    ax = plt.gca()
    # Do not change data range, only increase graph width to lengthen the physical ratio of the horizontal axis
    plt.tight_layout()
    ...
    # Select the first 3 samples as an example (if the data is sufficient)
    ...
    for sample_idx in range(n_samples_waterfall):
        ...
        ax = plt.gca()
        # Force display of negative sign and font
    ...
    # Create explanation dataset containing multiple samples for force plot
    ...
    # Specifically check the correlation between Xiao_strain and peak_strain
    if 'Xiao_strain' in feature_names:
        ...
        # Get the SHAP value of Xiao_strain
        ...
        print(f"\n🔍 Specifically check: Xiao_strain feature")
        print(f"  - Actual correlation with the target variable (peak_strain): {xiao_target_corr:+.4f}")
        if xiao_shap_mean is not None:
            ...
            if xiao_target_corr > 0 and xiao_shap_mean < 0:
                ...
                print(f"    - The actual correlation between Xiao_strain and peak_strain is positive ({xiao_target_corr:+.4f})")
                print(f"    - But the SHAP value shows a negative impact ({xiao_shap_mean:+.4f})")
                print(f"    - This may indicate:")
                print(f"      1. The model may have learned some compensation relationship (if the Xiao_strain prediction value is high, the model may think it needs to reduce the prediction)")
                print(f"      2. There is multicollinearity between features, causing the model to learn the wrong pattern")
                print(f"      3. The model may have overfitted the training data")
                print(f"    - Suggestions: check the correlation between features, consider removing Xiao_strain or adjusting the model")
            elif xiao_target_corr < 0 and xiao_shap_mean > 0:
                print(f"  ⚠ Warning:发现异常情况！")
                print(f"    - The actual correlation between Xiao_strain and peak_strain is negative ({xiao_target_corr:+.4f})")
                print(f"    - But the SHAP value shows a positive impact ({xiao_shap_mean:+.4f})")
    ...
    if len(low_importance_features) > 0:
        ...
        print("\nPossible reasons:")
        print("  1. The values of the features have a small range (small variance), resulting in limited impact on prediction")
        print("  2. The values of the features are highly correlated with other features, being replaced by other features")
        print("  3. The values of the features are distributed evenly in the dataset, lacking distinction")
        print("  4. This feature确实对目标变量影响较小（符合物理/工程规律）")
        ...
        print("  - View PDP plots to understand the marginal effects of these features")
...
def analyze_feature_statistics(X, y, feature_names, shap_importance, save_dir):
    ...
    # Create feature_stats in the order of shap_importance (consistent with the SHAP summary plot)
    ...
    for idx, row in shap_importance.iterrows():
        ...
        # Calculate the statistical information of this feature
        ...
        # Specifically check the w/c feature
        if feat_name == 'w/c':
            print(f"\n🔍 Specifically check the w/c feature:")
            ...
            if feat_range < 1e-6:
                print(f"  ⚠ Warning: w/c feature value range接近0！")
    ...
    # feature_stats is already sorted by shap_importance in descending order (consistent with the SHAP summary plot)
    
    # Save the statistical information (will be saved in the main function)
    # Return DataFrame for the main function to save
    
    # Analyze the features with small impact
    ...
    if len(low_importance) > 0:
        ...
        for idx, row in low_importance.iterrows():
            ...
            if abs(row['correlation_with_target']) < 0.1:
                reasons.append("The correlation with the target variable is very low")
            if row['cv'] < 0.1:
                reasons.append("The CV is small (the data has a small range)")
            if row['range'] < row['mean'] * 0.1:
                reasons.append("The range is very small compared to the mean")
            
            if reasons:
                print(f"    - Possible reasons: {', '.join(reasons)}")
    
    # Analyze the features with large impact
    ...
    for idx, row in high_importance.iterrows():
        print(f"  {row['feature']:10s}: SHAP={row['shap_importance']:.4f}, "
              f"Correlation with the target variable: {row['correlation_with_target']:+.4f}, CV: {row['cv']:.4f}")
    
    # Plot the feature statistics
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    
    # 1. SHAP importance vs correlation with the target variable
    ax1 = axes[0, 0]
    ...
    ax1.set_title('SHAP importance vs correlation with the target variable', fontsize=14, fontweight='bold')
    ...
    # Check and print the detailed information of the w/c feature
    ...
    if len(wc_feature) > 0:
        ...
        if wc_row['range'] < 1e-6:
            print(f"  ⚠ Warning: w/c feature value range is close to 0, it may be that all the values in the sample are the same!")
    ...
    # But keep them in top_features so that the user can see the problem
    ...
    if len(valid_range_features) < len(top_features):
        ...
        print(f"  We will use all features to plot (including features with range 0), to find the problem")
    
    y_pos = np.arange(len(top_features))
    # Plot the horizontal bar chart, the most important features are at the top (index 0)
    ...
    # For features with range 0, use a different color
    ...
    ax3.set_title('The value range of the top 10 important features (sorted by SHAP importance)', fontsize=14, fontweight='bold')
    ax3.grid(True, alpha=0.3, axis='x')
    # Invert the y-axis, so that the most important features are at the top (consistent with the SHAP summary plot: importance decreases from top to bottom)
    ax3.invert_yaxis()
    
    # If there are features with range 0, add a legend
    ...
    ax4 = axes[1, 1]
    # Calculate the absolute correlation and sort
...
def plot_pdp_analysis(model, X_train, feature_names, save_dir, n_top_features=5):
        ...
        try:
            ...
            for i in range(n_grid - 1):
                ...
                if np.sum(mask) > 0:
                    ...
                    # Set the feature value to the two ends of the interval
    ...
    # plt.show()  # Show the univariate PDP plot
    plt.close()
    
    print(f"Univariate PDP analysis completed, the dependency plot of the top {n_top_features} important features has been drawn")
    
    # 2. Bivariate PDP analysis (feature interaction)
    if n_top_features >= 2:
        print("\n=== Performing bivariate PDP analysis ===")
        
        # Select the top 4 most important features for bivariate interaction analysis
        ...
        # Plot the bivariate PDP (using heatmap)
        ...
        for idx, (feat1, feat2) in enumerate(interaction_pairs):
            try:
                ...
                # Calculate the bivariate partial dependence
                pd_result = partial_dependence(
                    model,
                    X_train_df,
                ...
                if average.shape != X1.shape:
                    # If the dimensions do not match, transpose or reshape
                ...
                # For 3D plots, check if it is a subplot
                if idx < len(axes):
        ...
        print(f"Bivariate PDP analysis completed, the interaction plot of the top {n_interaction_features} important features has been drawn")
...
def analyze_noise_robustness(model, X_test, y_test, feature_names, save_dir):
    ...
    # 1. Test the robustness of the model to Gaussian noise
    print("Test the robustness of the model to Gaussian noise...")
    # Use 0, 2, 4, 6, 8, 10 (%) as the noise levels
    ...
    # 2. Test the robustness of the model to outliers
    print("Test the robustness of the model to outliers...")
    # Use 0, 2, 4, 6, 8, 10 (%) as the outlier levels
    outlier_levels = [0, 0.02, 0.04, 0.06, 0.08, 0.10]  # Outlier levels (percentage of samples)
    outlier_severity = 5.0  # Outlier severity (multiple of the standard deviation)
    outlier_rmses = []
    outlier_stds = []
    ...
    # 3. Plot the robustness analysis figure
    ...
    # Save the results (will be saved in the main function)
    ...
    print(f"   - 0% noise level: {noise_rmses[0]:.3f}")
    print(f"   - 2% noise level: {noise_rmses[1]:.3f} (increase {noise_rmses[1]-noise_rmses[0]:.3f})")
    print(f"   - 4% noise level: {noise_rmses[2]:.3f} (increase {noise_rmses[2]-noise_rmses[0]:.3f})")
    print(f"   - 6% noise level: {noise_rmses[3]:.3f} (increase {noise_rmses[3]-noise_rmses[0]:.3f})")
    print(f"   - 8% noise level: {noise_rmses[4]:.3f} (increase {noise_rmses[4]-noise_rmses[0]:.3f})")
    print(f"   - 10% noise level: {noise_rmses[5]:.3f} (increase {noise_rmses[5]-noise_rmses[0]:.3f})")
    
    print("\n2. Outliers robustness:")
    print(f"   - 0% outlier level: {outlier_rmses[0]:.3f}")
    print(f"   - 2% outlier level: {outlier_rmses[1]:.3f} (change {outlier_rmses[1]-outlier_rmses[0]:.3f})")
    print(f"   - 4% outlier level: {outlier_rmses[2]:.3f} (change {outlier_rmses[2]-outlier_rmses[0]:.3f})")
    print(f"   - 6% outlier level: {outlier_rmses[3]:.3f} (change {outlier_rmses[3]-outlier_rmses[0]:.3f})")
    print(f"   - 8% outlier level: {outlier_rmses[4]:.3f} (change {outlier_rmses[4]-outlier_rmses[0]:.3f})")
    print(f"   - 10% outlier level: {outlier_rmses[5]:.3f} (change {outlier_rmses[5]-outlier_rmses[0]:.3f})")
    
    # Calculate the performance degradation rate
    ...
    print(f"   - Robustness to Gaussian noise: {noise_robust} (10% noise level: performance degradation {noise_degradation:.1f}%)")
    print(f"   - Robustness to outliers: {outlier_robust} (10% outliers level: performance change {outlier_degradation:.1f}%)")
...
def analyze_prediction_intervals(model, X_test, y_test, save_dir, n_bootstrap=100, bootstrap_noise_level=0.01):
    ...
    # 3. Draw the prediction interval comparison figure
    ...
    # Invert the y-axis to make Bootstrap at the top
    ax3.invert_yaxis()
    ...
    # Save the prediction interval results
...
def load_trained_model(model_path):
    """Load the trained XGBoost model (supports .joblib and .pkl format)"""
    ...
    if feature_names:
        print(f"✓ Feature数量: {len(feature_names)}")
        print(f"✓ Feature names: {feature_names}")
    else:
        print("⚠ Warning: Feature names not found, using feature names when loading data")
...
def cross_subset_validation(X, y, feature_names, sample_divisions, sample_ids, original_df_indices, df_processed, 
    ...
    for i, val_label in enumerate(subset_labels):
        ...
        # Create the save directory for this round
    ...
    # Calculate the three-fold average metrics
    ...
    # Use the best hyperparameters, retrain the final model with all subset data (excluding test)
    print(f"\n{'='*60}")
    ...
    print(f"Final model from fold {best_fold_result['val_label']} validation set.")
    ...
    # 1. Plot the results figure
    plot_results(y_test_final, y_test_pred, interpretability_dir)
    
    # 2. Plot the training set and test set comparison figure
    ...
    try:
        ...
        # Create the prediction result dictionary
        pred_dict = {}
        ...
        if 'No_Customized' in df_original.columns:
            df_original['XGBoost_peak_strain'] = df_original['No_Customized'].map(pred_dict)
        else:
            print("Warning: No_Customized column not found, cannot match prediction results")
        
        # Save the results
        output_file = os.path.join(PROJECT_ROOT, "dataset/dataset_with_XGBoost_peak_strain.xlsx")
        df_original.to_excel(output_file, index=False)
        print(f"✓ Prediction results written to: {output_file}")
        print(f"  - New column: XGBoost_peak_strain (final model prediction value)")
        print(f"  - Non-empty prediction value number: {df_original['XGBoost_peak_strain'].notna().sum()}")
        
        # Backup
        backup_file = os.path.join(save_dir, 'dataset_with_XGBoost_peak_strain_backup.xlsx')
...
def cross_random_kfold_validation(X, y, feature_names, sample_ids, sample_divisions=None,
    ...
    for res in all_results:
        if res['metrics']:
            print(f"[Fold {res['fold_idx']}] Test set metrics: R2={res['metrics']['r2']:.4f}, MAE={res['metrics']['mae']:.3f}, RMSE={res['metrics']['rmse']:.3f}")
    ...
    try:
        ...
        if 'No_Customized' in df_original.columns:
            df_original['XGBoost_peak_strain_random'] = df_original['No_Customized'].map(pred_dict)
        else:
            df_original['XGBoost_peak_strain_random'] = y_all_pred

        output_file = os.path.join(save_dir, "dataset_with_XGBoost_peak_strain_random.xlsx")
        df_original.to_excel(output_file, index=False)
        print(f"✓ Random split model prediction results written to: {output_file}")
        print(f"  - New column: XGBoost_peak_strain_random")
        print(f"  - Non-empty prediction value number: {df_original['XGBoost_peak_strain_random'].notna().sum()}")
    except Exception as e:
...
def main():
    ...
    print("=== XGBoost SHAP analysis, noise analysis and uncertainty analysis ===")
    ...
    # 2. Load the trained model - directly using xgboost_final_model.joblib
    model_path = os.path.join(SAVE_ROOT, 'xgboost_final_model.joblib')
    
    if not os.path.exists(model_path):
        print(f"Error: Model file not found: {model_path}")
        print("Please ensure the model file exists in: peak_strain/XGBoost/save/xgboost_final_model.joblib")
        return
    ...
    if 'feature_names' in model_data and model_data['feature_names'] is not None:
        ...
        if len(model_feature_names) == len(feature_names):
            ...
            if set(model_feature_names) == set(feature_names):
                ...
                print("⚠ Warning: Model feature names do not match data feature names completely")
    ...
    # 10. PDP analysis (using training set + validation set, if data is large, it may be slow)
    print(f"\n{'='*60}")
    ...
    if noise_rmses is not None:
        ...
        print(f"  - No noise时RMSE: {noise_rmses[0]:.3f}")
        print(f"  - 10% noise时RMSE: {noise_rmses[-1]:.3f}")
