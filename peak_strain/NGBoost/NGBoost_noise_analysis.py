...
# -*- coding: utf-8 -*-
"""
NGBoost Model Noise Analysis and Uncertainty Analysis - Peak Strain Prediction

Features:
1. Load pre-trained NGBoost model
2. Noise robustness analysis: Test model robustness to Gaussian noise and outliers
3. Prediction interval analysis: Generate confidence intervals using NGBoost's built-in probabilistic prediction and Bootstrap method
4. SHAP analysis: Feature importance analysis
5. PDP analysis: Partial dependence plot analysis

Note: This script requires running NGBoost_train.py first to train the model, then load the saved model for analysis
This script is used for peak strain prediction, using 17 features (15 material parameters + fc + Xiao_strain)
...
# Set plot fonts and LaTeX rendering
def configure_plot_fonts(fonts=None):
    """Unified Matplotlib font settings to ensure proper display of negative signs"""
    if fonts is None:
...
plt.rcParams['mathtext.default'] = 'regular'   # Use regular font for mathematical symbols

# Working directory settings - automatically find project root directory
def find_project_root():
    ...
    try:
        ...
        # If script is in subdirectory, find up to directory containing dataset or SAVE
        ...
        # Running in Jupyter, search up from current directory
        pass
    
    # Search up from current working directory
    current_dir = os.path.abspath(os.getcwd())
    search_limit = 0
    while current_dir != os.path.dirname(current_dir) and search_limit < 10:
        # Check if contains dataset folder or specific project files
        if os.path.exists(os.path.join(current_dir, 'dataset')) or \
           os.path.exists(os.path.join(current_dir, 'SAVE')):
            return current_dir
        current_dir = os.path.dirname(current_dir)
...
def load_data():
    """Load data - Regression of peak strain using 15 material parameters + peak stress(fc) + Xiao_strain"""
    print("=== Loading Data ===")
    ...
    # Read first worksheet of Excel file (material parameter table)
    ...
    if missing_cols:
        ...
        if "Xiao_strain" in missing_cols and "fc" in df.columns and "r" in df.columns:
            ...
            print("  ✓ Xiao_straincalculated and added")
        ...
        if missing_cols:
            ...
            print(f"   Removed missing features: {missing_cols}，current feature count: {len(feature_names)}")
    ...
    # Special check for w/c feature (if exists)）
    if 'w/c' in feature_names:
        wc_idx = feature_names.index('w/c')
        wc_values = X[:, wc_idx]
        print(f"\n🔍 Checking w/c feature (raw data):")
        print(f"  Number of unique values: {len(np.unique(wc_values))}")
        ...
        if np.max(wc_values) - np.min(wc_values) < 1e-6:
            print(f"  ⚠ Warning: w/c feature range is close to 0 in raw data!")
    
    # Check for missing values
    if np.isnan(X).any():
        print(f"Warning: Feature data contains NaN values, will impute")
    ...
    if 'DataSlice' in df.columns:
        ...
        print(f"Found dataset division information: {np.unique(sample_divisions, return_counts=True)}")
    else:
    ...
    print(f"Number of features: {X.shape[1]} (15 material parameters + fc + Xiao_strain = 17 features)")
    print(f"Number of samples: {X.shape[0]}")
    print(f"Target variable '{target_column}' range: {np.min(y):.2f} - {np.max(y):.2f}")
    
    # Save original DataFrame indices (for mapping prediction results later)
...
def split_data_by_divisions(X, y, sample_divisions, sample_ids, test_ratio=0.2, val_ratio=0.2, random_state=42):
    ...
    print("=== Splitting Data Based on Sample Division Information ===")
    ...
    # Classify samples based on division information (supports DataSlice format: subset1, subset2, subset3, test)
    train_indices = []
    val_indices = []
    test_indices = []
    ...
    for i, division in enumerate(sample_divisions):
        ...
        if division_str.lower().startswith('test'):
            ...
            # subset1, subset2, subset3 all as training set (or can be allocated as needed)
            # Default to all as training set, can split validation set from subset if needed
            train_indices.append(i)
        elif division == 'train' or division_str.startswith('train'):
            train_indices.append(i)
        elif division == 'val' or division == 'validation' or division_str.startswith('val'):
            val_indices.append(i)
        else:
            # Default to training set
            train_indices.append(i)
    
    # If no validation set but has training set, split validation set from training set
    # Test set remains unchanged if exists
    if train_indices and not val_indices:
        print("Only training set samples found, splitting validation set from training set...")
        from sklearn.model_selection import train_test_split
        ...
        if len(train_indices) > 1:
            ...
            # If test set does not exist, split validation and test sets from training set
            if len(test_indices) > 0:
                # Test set exists, only split validation set
                ...
                # Test set does not exist, split validation and test sets from training set
...
# Note: Analysis script does not need objective function as model is already trained

# Note: Analysis script does not need optimization and training functions as model is already trained

def plot_results(y_true, y_pred, save_dir, model_name='ngboost'):
    """Plot results"""
    print("\n=== Plotting Results ===")
    ...
    fig.suptitle(f'NGBoost Results ($R^2$={r2:.4f})', fontsize=16, fontweight='bold')
...
- Feature Importance Analysis"""
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'ngboost_results.png'), dpi=300, bbox_inches='tight')
    plt.close()
    
    return r2, mae, rmse

def plot_train_test_comparison(model, X_train, y_train, X_test, y_test, save_dir):
    """Plot training and test set comparison (with edge distribution) - optimized version
    
    Returns:
        DataFrame: DataFrame containing training and test set prediction results
    """
    print("\n=== Plot training and test set comparison ===")
    ...
    # Define grid layout (adjust proportions)
    ...
    # Add total title - containing sample number information
    title = f'NGBoost Model Performance\nTraining: n={len(y_train)} | Testing: n={len(y_test)}'
...
def plot_feature_importance(model, feature_names, save_dir, X=None):
    ...
    # Get feature importance (NGBoost uses feature_importances_ attribute)
    try:
        importance = model.feature_importances_
    except AttributeError:
        # If NGBoost does not have feature_importances_, use SHAP values or manually calculate
        print("⚠ NGBoost model does not have feature_importances_ attribute, will use SHAP values to calculate feature importance")
        # Use a small background dataset to calculate SHAP values
        if X is not None and len(X) > 0:
            ...
            # If cannot calculate, use uniform importance
    ...
    plt.title('NGBoost Feature Importance')
...
# NGBoost model does not need to be repaired, directly use it
...
def plot_shap_analysis(model, X_background, X_explain, feature_names, save_dir, y_explain=None):
    ...
    Args:
        ...
        feature_names: Feature name list
        save_dir: Save directory
        y_explain: Target variable value (optional, for checking feature-target variable correlation)
    ...
    Traditional做法说明:
        ...
        - Explanation dataset (X_explain): Using test set
          Explain the unseen data, evaluate the interpretability of model generalization ability
        
        优点：
        1. Conforms to machine learning best practices (training/test separation)
        2. Evaluate the interpretability of model in real application scenarios
    ...
    # NGBoost model can use TreeExplainer (because it uses decision trees under the hood)
    # If the background dataset is passed in error, it can be passed in without background dataset (using default value)
    try:
        # Try to use background dataset to initialize explainer
        explainer = shap.TreeExplainer(model, X_background)
        print("✓ Background dataset successfully initialized explainer")
    except (ValueError, TypeError, AttributeError) as e:
        print(f"⚠ Background dataset failed to initialize explainer: {e}")
        print("  Trying to initialize explainer without background dataset...")
        try:
            # Do not pass in background dataset, use default value
            explainer = shap.TreeExplainer(model)
            print("✓ Background dataset successfully initialized explainer")
        except Exception as e2:
            print(f"⚠ TreeExplainer initialization failed: {e2}")
            ...
            try:
                ...
                else:background_sample = X_background
                explainer = shap.KernelExplainer(model.predict, background_sample)
                print("✓ KernelExplainer successfully initialized (note: slower calculation speed)")
    ...
    # For SHAP plot, use DejaVu Sans to ensure negative sign is displayed correctly
    ...
    # 1. Summary plot
    # Significantly increase width, truly lengthen the horizontal axis ratio
    fig = plt.figure(figsize=(30, 12))  # Width increased to 30, significantly lengthen the horizontal axis
    shap.summary_plot(shap_values, X_explain, feature_names=feature_names, show=False)
    # Get current axes
    ax = plt.gca()
    # Do not change data range, only increase graphic width to lengthen horizontal axis physical ratio
    plt.tight_layout()
    ...
    for sample_idx in range(n_samples_waterfall):
        ...
        # Ensure all text uses font that supports negative sign
    ...
    print(f"✓ {n_samples_waterfall} Waterfall plots generated")
    ...
    # Draw force plot for first 3 samples
    ...
    if 'Xiao_strain' in feature_names:
        ...
        # Calculate the actual correlation between Xiao_strain and target variable
        xiao_target_corr = np.corrcoef(xiao_values, y_explain)[0, 1]
        # Get SHAP value for Xiao_strain
        ...
        if xiao_shap_mean is not None:
            ...
            if xiao_target_corr > 0 and xiao_shap_mean < 0:
                print(f"  ⚠ Warning: Found abnormal情况！")
                print(f"    - Actual correlation between Xiao_strain and peak_strain is positive ({xiao_target_corr:+.4f})")
                ...
                print(f"  ⚠ Warning: Found abnormal情况！")
                print(f"    - Actual correlation between Xiao_strain and peak_strain is negative ({xiao_target_corr:+.4f})")
    ...
    # Analyze the possible reasons why features have a small impact
    low_importance_features = shap_importance[shap_importance['shap_importance'] < 0.1]
    if len(low_importance_features) > 0:
        print(f"\n⚠ Found {len(low_importance_features)} features with small impact (average SHAP value < 0.1):")
        ...
        print("  4. The feature has a small impact on the target variable (符合物理/工程规律）")
        print("  5. The sample size is small (94 samples), the influence of some features may be masked by noise")
        print("\nSuggestions:")
...
def analyze_feature_statistics(X, y, feature_names, shap_importance, save_dir):
    ...
    # Create feature_stats according to the order of shap_importance (consistent with SHAP summary plot)
    ...
    for idx, row in shap_importance.iterrows():
        ...
        if feat_name == 'w/c':
            ...
            if feat_range < 1e-6:
                ...
                print(f"    2. All values are the same after w/c is standardized in data preprocessing")
    ...
    # Analyze features with small impact
    low_importance_threshold = 0.1
    low_importance = feature_stats[feature_stats['shap_importance'] < low_importance_threshold]
    
    print(f"\nAnalysis of features with small impact (SHAP importance < {low_importance_threshold}):")
    if len(low_importance) > 0:
        print(f"\nThere are {len(low_importance)} features with small impact:")
    ...
    for idx, row in high_importance.iterrows():
        print(f"  {row['feature']:10s}: SHAP={row['shap_importance']:.4f}, "
              f"Correlation with target variable={row['correlation_with_target']:+.4f}, CV={row['cv']:.4f}")
    
    # Plot feature statistics
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    
    # 1. SHAP importance vs correlation with target variable
    ax1 = axes[0, 0]
    ...
    ax1.set_title('SHAP importance vs correlation with target variable', fontsize=14, fontweight='bold')
    ...
    ax3 = axes[1, 0]
    # feature_stats is already sorted by SHAP importance in descending order, take the top 10
    # Note: SHAP summary plot is sorted from top to bottom by importance, so the most important is at the top
    ...
    if len(wc_feature) > 0:
        ...
        if wc_row['range'] < 1e-6:
            print(f"  ⚠ Warning: w/c feature value range is close to 0, possibly all samples have the same value!")
            print(f"  This will cause SHAP importance to be 0, because the feature value cannot affect the prediction when it is constant.")
    ...
    # But keep them in top_features to allow users to see the problem
    ...
    if len(valid_range_features) < len(top_features):
        ...
        print(f"  All features will be plotted (including features with range 0) to discover the problem")
    ...
    ax3.set_title('Range of the 10 most important features (sorted by SHAP importance)', fontsize=14, fontweight='bold')
    ax3.grid(True, alpha=0.3, axis='x')
    # Invert y axis, so the most important features are at the top (consistent with SHAP summary plot: importance decreases from top to bottom)
    ax3.invert_yaxis()
    
    # If there are features with range 0, add a legend to explain
    ...
    # 4. Sort by correlation (sorted by absolute value)
    ax4 = axes[1, 1]
    ...
    ax4.set_title('Correlation with target variable (absolute value of the top 10)', fontsize=14, fontweight='bold')
...
def plot_pdp_analysis(model, X_train, feature_names, save_dir, n_top_features=5):
    ...
    try:
        ...
        # If NGBoost does not have feature_importances_, use SHAP values to calculate feature importance
        print("⚠ NGBoost model does not have feature_importances_ attribute, using SHAP values to calculate feature importance")
        background_size = min(50, len(X_train))
        background = X_train[:background_size] if isinstance(X_train, np.ndarray) else X_train.iloc[:background_size]
        explainer = shap.TreeExplainer(model)
        shap_values = explainer.shap_values(background)
        importance = np.abs(shap_values).mean(axis=0)
    ...
    for idx, (feature, ax) in enumerate(zip(top_features, axes)):
        try:
            ...
            # Calculate the mean of ICE curves (for Mean c-ICE)
            ...
            # ALE is calculated by calculating local effects and accumulating them
    ...
    if n_top_features >= 2:
        ...
        # Select the 4 most important features for double variable interaction analysis
        ...
        n_cols = 3  # Change to 3 columns for better美观
        ...
        for idx, (feat1, feat2) in enumerate(interaction_pairs):
            try:
                ...
                # For 3D plot, need to check if it is a subplot
                if idx < len(axes):
...
def analyze_noise_robustness(model, X_test, y_test, feature_names, save_dir):
    ...
    print(f"   - 0%噪声时RMSE: {noise_rmses[0]:.3f}")
    print(f"   - 2% noise RMSE: {noise_rmses[1]:.3f} (increase {noise_rmses[1]-noise_rmses[0]:.3f})")
    print(f"   - 4% noise RMSE: {noise_rmses[2]:.3f} (increase {noise_rmses[2]-noise_rmses[0]:.3f})")
    print(f"   - 6% noise RMSE: {noise_rmses[3]:.3f} (increase {noise_rmses[3]-noise_rmses[0]:.3f})")
    print(f"   - 8% noise RMSE: {noise_rmses[4]:.3f} (increase {noise_rmses[4]-noise_rmses[0]:.3f})")
    print(f"   - 10% noise RMSE: {noise_rmses[5]:.3f} (increase {noise_rmses[5]-noise_rmses[0]:.3f})")
    
    print("\n2. Outlier robustness:")
    print(f"   - 0% outlier RMSE: {outlier_rmses[0]:.3f}")
    print(f"   - 2% outlier RMSE: {outlier_rmses[1]:.3f} (change {outlier_rmses[1]-outlier_rmses[0]:.3f})")
    print(f"   - 4% outlier RMSE: {outlier_rmses[2]:.3f} (change {outlier_rmses[2]-outlier_rmses[0]:.3f})")
    print(f"   - 6% outlier RMSE: {outlier_rmses[3]:.3f} (change {outlier_rmses[3]-outlier_rmses[0]:.3f})")
    print(f"   - 8% outlier RMSE: {outlier_rmses[4]:.3f} (change {outlier_rmses[4]-outlier_rmses[0]:.3f})")
    print(f"   - 10% outlier RMSE: {outlier_rmses[5]:.3f} (change {outlier_rmses[5]-outlier_rmses[0]:.3f})")
...
def analyze_prediction_intervals(model, X_test, y_test, save_dir, n_bootstrap=100, bootstrap_noise_level=0.01):
    ...
    2. Quantile Regression: Build fixed-width prediction intervals based on residual distribution
    """
    ...
    for _ in range(n_bootstrap):
        ...
        # Add small amount of noise to the test data to reflect the model's response to input uncertainty
    ...
    # 2. Quantile Regression prediction intervals (using residual distribution of training data)
    print("Generating Quantile Regression prediction intervals...")
    y_pred_test = model.predict(X_test)
    residuals = y_test - y_pred_test
    residual_std = np.std(residuals)
    
    # Build prediction intervals using the standard deviation of residuals
    ...
    # 3. Plot prediction interval comparison graph
    ...
    # Bootstrap prediction intervals (top left)
    ...
    # Quantile Regression prediction intervals (bottom left)
    ...
    bars2 = ax3_twin.barh(y_pos + height/2, widths, height, 
                         label='Avg Interval Width', color='coral', alpha=0.8)
...
def train_single_fold(X_train, y_train, X_val, y_val, X_test, y_test, train_ids, val_ids, test_ids, 
    ...
    Note: This function is usually not called in the analysis script, because the analysis script is mainly used to load trained models.
    If you need to use this function, you need to implement the optimize_ngboost and train_ngboost functions.
    """
    # Note: The analysis script is mainly used to load trained models, this function is reserved for possible cross-validation needs
    # If you need to use it, you need to implement the optimize_ngboost and train_ngboost functions
    raise NotImplementedError("The analysis script is mainly used to load trained models, if you need cross-validation please use the training script")
...
def load_trained_model(model_path):
    """Load trained NGBoost model (supports .joblib and .pkl format)"""
    print(f"\n=== Load trained model ===")
    ...
    if feature_names:
        ...
        print("⚠ Warning: No feature names found, using feature names from data loading")
...
def cross_subset_validation(X, y, feature_names, sample_divisions, sample_ids, original_df_indices, df_processed, 
    ...
    print("\n=== NGBoost Subset three-fold cross-validation ===")

    if save_dir is None:
        save_dir = os.path.join(SAVE_ROOT, 'ngboost_cv')
    ...
    for i, val_label in enumerate(subset_labels):
        ...
        # Train single fold model (only hyperparameter optimization and training, no interpretability analysis)
        ...
        print(f"[{val_label} Validation set] This round training completed")
        if metrics:
    ...
    # Evaluate on test set (should be consistent with the metrics of this fold)
    ...
    print(f"Final model from validation set {best_fold_result['val_label']} round.")
    ...
    # 2. Plot training set and test set comparison
    ...
    }, os.path.join(interpretability_dir, 'final_ngboost_model.pkl'))
    ...
    try:
        ...
        if 'No_Customized' in df_original.columns:
            df_original['NGBoost_peak_strain'] = df_original['No_Customized'].map(pred_dict)
        else:
            print("⚠ Warning: No_Customized column not found, cannot match prediction results")
        
        # Save results
        output_file = os.path.join(PROJECT_ROOT, "dataset/dataset_with_NGBoost_peak_strain.xlsx")
        df_original.to_excel(output_file, index=False)
        print(f"✓ Prediction results written to: {output_file}")
        print(f"  - New column: NGBoost_peak_strain (final model predicted value)")
        print(f"  - Non-empty prediction value number: {df_original['NGBoost_peak_strain'].notna().sum()}")
        
        # Backup
        backup_file = os.path.join(save_dir, 'dataset_with_NGBoost_peak_strain_backup.xlsx')
...
def cross_random_kfold_validation(X, y, feature_names, sample_ids, sample_divisions=None,
    ...
    print(f"\n=== NGBoost Random {n_splits} fold cross-validation ===")

    if save_dir is None:
        save_dir = os.path.join(SAVE_ROOT, 'ngboost_cv_random')
    ...
    for fold_idx, (train_indices_rel, val_indices_rel) in enumerate(kf.split(split_source), 1):
        print(f"\n{'='*60}")
        print(f"Random KFold round {fold_idx}/{n_splits}")
        print(f"{'='*60}")
    ...
    print(f"Final model using Fold {best_fold_result['fold_idx']} training.")
    print(f"  R2: {final_r2:.4f}")
    print(f"  MAE: {final_mae:.3f}")
    print(f"  RMSE: {final_rmse:.3f}")
    if avg_r2 is not None:
    ...
    joblib.dump({
        ...
        'shap_importance': shap_importance,
        'cross_validation_results': all_results
    }, os.path.join(interpretability_dir, 'final_ngboost_model_random.pkl'))
    ...
    try:
        ...
        if 'No_Customized' in df_original.columns:
            df_original['NGBoost_peak_strain_random'] = df_original['No_Customized'].map(pred_dict)
        else:
            df_original['NGBoost_peak_strain_random'] = y_all_pred

        output_file = os.path.join(save_dir, "dataset_with_NGBoost_peak_strain_random.xlsx")
        df_original.to_excel(output_file, index=False)
        print(f"✓ Random split model prediction results written to: {output_file}")
        print(f"  - New column: NGBoost_peak_strain_random")
        print(f"  - Non-empty prediction value number: {df_original['NGBoost_peak_strain_random'].notna().sum()}")
    except Exception as e:
...
def main():
    ...
    print("=== NGBoost SHAP analysis, noise analysis and uncertainty analysis ===")
    ...
    # 2. Load trained model - directly use ngboost_final_model.joblib
    model_path = os.path.join(SAVE_ROOT, 'ngboost_final_model.joblib')
    
    if not os.path.exists(model_path):
        print(f"⚠ Error: Model file not found: {model_path}")
        print("Ensure the model file exists in: peak_strain/NGBoost/save/ngboost_final_model.joblib")
        return
    
    model, model_data = load_trained_model(model_path)
    
    # If model data has feature names, use the model's (ensure feature order is consistent)
    if 'feature_names' in model_data and model_data['feature_names'] is not None:
        model_feature_names = model_data['feature_names']
        print(f"\nUse model saved feature names ({len(model_feature_names)} features)")
    ...
    # Prepare data set for SHAP analysis
    ...
    print(f"\n{'='*60}")
    print("Start plot prediction result comparison")
    print(f"{'='*60}")
    prediction_df = None
    try:
        y_test_pred = model.predict(X_test)
    ...
    if noise_rmses is not None:
        ...
        print(f"  - RMSE without noise: {noise_rmses[0]:.3f}")
        print(f"  - RMSE with 10% noise: {noise_rmses[-1]:.3f}")
        print(f"  - Performance degradation: {(noise_rmses[-1] - noise_rmses[0]) / noise_rmses[0] * 100:.1f}%")
    ...
    # 11. Save all analysis results to an Excel file in different sheets
    print(f"\n{'='*60}")
    print("Save all analysis results to an Excel file in different sheets")
    print(f"{'='*60}")
    try:
