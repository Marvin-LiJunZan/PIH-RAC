...
It does not include inference and detailed evaluation functions, use peak_strain_inference.py for inference.
...
- Early stopping: validation set performance does not improve then terminate training
...
✅ Optuna hyperparameter optimization (optional):
   - Each Trial's hyperparameter configuration
   - Mean and standard deviation of validation R² for each fold
   - Best Trial's hyperparameters and validation R²
...
os.makedirs(os.path.normpath(r"C:\JunzanLi_project\constitutive_relation\Pi_BiLSTM\peak_strain\SAVE"), exist_ok=True)
...
# Base configuration dataclass. Will be expanded as needed.
@dataclass
class BaseConfig:
    """Base configuration dataclass. Will be expanded as needed."""
    ...
    save_dir: str = os.path.normpath(r"C:\JunzanLi_project\constitutive_relation\Pi_BiLSTM\peak_strain\SAVE")
    ...
    num_workers: int = 0  # Windows/laptop recommended 0
...
class TrainingConfig:
    ...
    hidden_dim: int = 512  # Increased to 512 (best was 407 which is close to 512)
    num_layers: int = 6    # Increased to 6 layers (best was 6)
    
    # Regularization strategy (Moderate regularization, do not over-restrict)
    dropout: float = 0.15          # Decreased dropout (too high will limit learning ability)
    weight_decay: float = 5e-5     # Decreased L2 regularization
    
    # Learning rate and optimization strategyLearning rate and optimization strategy
    learning_rate: float = 2e-3    # Increased initial learning rate
    use_scheduler: bool = True     # Use learning rate decay
    scheduler_gamma: float = 0.97  # Moderately decreased
...
class StrainDataset(Dataset):
    ...
    def __init__(
        ...
        if r_values is not None:
            # r = Recycled Aggregate replacement rate, divided by 100: from percentage × 100 (e.g. 1.5 represents 1.5%) to decimal (0.015)
...
def _xiao_formula_core(stress, r, lib):
    """Xiao formula core calculation logic (NumPy/PyTorch universal).
    
    公式：ε_cp = {0.00076 + [(0.626σ_cp - 4.33) × 10^-7]^0.5} × (1 + r / (65.715r^2 - 109.43r + 48.989))
    
    Args:
        stress: Stress value (fc)
        r: Recycled Aggregate replacement rate (decimal form, e.g. 0.015 represents 1.5%)
    ...
    if is_numpy:
        # NumPy uses clip, the parameter name is a_min/a_max
        ...
        # PyTorch uses clamp, the parameter name is min/max
...
def compute_xiao_formula(df: pd.DataFrame, stress_column: str) -> np.ndarray:
    ...
    # r value divided by 100: from percentage × 100 (e.g. 1.5 represents 1.5%) to decimal (0.015)
    r = r / 100.0
...
def load_dataframe(config: BaseConfig) -> pd.DataFrame:
    ...
    logging.info("Successfully read data, shape: %s", df.shape)
    return df


def preprocess_data(config: BaseConfig) -> Dict[str, Any]:
    """Data loading and preprocessing, returning information including all samples and DataSlice indices."""
    ...
    if config.baseline_column not in df.columns:
        logging.warning("Baseline column '%s' not found, baseline information will not be used", config.baseline_column)
    ...
    if dropped > 0:
        logging.warning("There are missing data, %d records have been deleted", dropped)
    ...
    # Extract r parameter for physical loss calculation (if it exists)
    if "r" in df_clean.columns:
        ...
        r_values = None
        logging.warning("r column is missing in the data, Xiao formula will not be able to calculate correctly! Please check the data file.")
    if "DataSlice" not in df_clean.columns:
        raise ValueError("DataSlice column is missing in the dataset, cannot split the dataset according to the specified method.")
    ...
    for idx, division in enumerate(sample_divisions_raw):
        ...
        elif lower_div.startswith("train") or division in {"Training set"}:
    ...
    data_dict: Dict[str, Any] = {
        ...
        "r_values": r_values,  # Add r parameter
...
class PINNRegressor(nn.Module):
    ...
    def __init__(
        ...
        if activation is None:
            # MinMaxScaler normalized to [0,1], ReLU更适合（输出[0,+∞)）
            # Output layer uses Sigmoid to ensure the final output is within the range [0,1]
            activation = nn.ReLU()
        ...
        # Output layer: uses Sigmoid to ensure the output is within the range [0,1], matching the normalization range of MinMaxScaler
        layers.append(nn.Linear(in_dim, 1))
        layers.append(nn.Sigmoid())  # Ensure the output is within the range [0,1]
...
def xiao_equation(stress: torch.Tensor, r: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        ...
        r: Recycled Aggregate replacement rate (decimal form, e.g. 0.015 represents 1.5%)
    Note: Input is converted from percentage × 100 to decimal (divided by 100)
...
def compute_physics_loss(
    ...
    Args:
        ...
        target_data_min: Minimum value of the target value (for inverse normalization)
        target_range: Range of the target value (for inverse normalization)
        r_values: Reinforcement ratio parameter (decimal form, e.g. 0.015 represents 1.5%)
    """
    # MinMaxScaler inverse normalization: x = x_scaled * (data_max_ - data_min_) + data_min_
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        r = batch.get("r")  # Get r parameter (if it exists)
        if r is not None:
...
def train_pinn(
    ...
    """Train the PINN model and return the training process and results.
    ...
    if "train" not in datasets or "val" not in datasets:
        raise ValueError("train_pinn needs to provide both training set and validation set indices.")
    ...
    # MinMaxScaler inverse normalization formula: x = x_scaled * (data_max_ - data_min_) + data_min_
    # Note: min_ and scale_ are internal properties, not the minimum and range of the original data
    # Should use data_min_ and data_max_ to calculate inverse normalization
    ...
    # Calculate range for inverse normalization
    ...
    for epoch in range(1, train_config.max_epochs + 1):
        ...
        # Note: pruning is not used in cross-validation, because it will cause repeated step reporting warnings
        # pruning is only useful in single training, cross-validation has already been evaluated at the fold level
        if trial is not None:
            # In cross-validation scenario, intermediate results are not reported during training
            # Only report the final results of each fold in the objective function
            pass
    ...
    if best_state is None:
        raise RuntimeError("Best model state was not recorded during training.")
...
def sample_training_config(trial: Trial, base_train_config: TrainingConfig) -> TrainingConfig:
    ...
    Optimization strategy (target: restore 0.87 R² performance):
    ...
    # ==================== Network structure ====================
    # Hidden layer dimension: expanded to 64-1024 (previously best 407, now contains larger range)
    cfg.hidden_dim = trial.suggest_int("hidden_dim", 64, 1024, step=32)
    
    # Hidden layer number: 2-10 layers (previously best 6 layers, expanded search space)
    ...
    # L2 regularization: 1e-6 to 1e-3 (expand search range)
    ...
    # Learning rate decay: 0.90-0.999 (expand range, previously best 0.987接近上限)
    ...
    # Although Xiao formula R²<0, physical constraints may act as regularization
...
def optimize_hyperparameters(
    ...
    # Get fixed test set and samples for cross-validation
    ...
    def objective(trial: Trial) -> float:
        """Optuna optimization objective: maximize the mean validation set R²."""
        try:
            ...
            for fold_idx, (train_idx_local, val_idx_local) in enumerate(kfold.split(train_val_indices), start=1):
                ...
                # Train model (logging is controlled in train_pinn)
                results = train_pinn(
                    data_dict,
                    base_config,
                    train_cfg,
            ...
            # Report final results to Optuna (for recording, not for pruning)
            # Use trial.number as step, avoid repeated reporting
            trial.report(mean_score, trial.number)
            
            # Output trial results
            logging.info("✓ Trial %d 完成: 平均验证R² = %.4f ± %.4f", 
    ...
    # Principle: maintain two probability models (good configurations and bad configurations), smartly select the next trial point
    sampler = TPESampler(
        n_startup_trials=10,  # First 10 trials randomly sampled (fully explore, establish good prior)
        n_ei_candidates=50,   # Increase to 50 (more precise expected improvement estimation)
        multivariate=True,    # Consider the correlation between hyperparameters (capture the interaction between hidden_dim and num_layers)
    ...
    # Reason: each fold is independently trained in cross-validation, pruning will cause repeated step reporting warnings
    # Solution: do not use pruner, let all trials run fully
    pruner = None  # Disable pruning, avoid warnings
    ...
    logging.info("Optimization target: restore to 0.87+ R² performance (Xiao formula R²~0.42)")
...
def _build_final_split_indices(
    ...
    if all_subset_indices.size < 2:
        raise ValueError("Subset sample number insufficient to divide into training and validation set.")
...
def train_with_subset_strategy(
    ...
    """Use predefined subset strategy for three-fold cross-validation, and collect detailed metrics."""
    ...
    if len(subset_labels) < 2:
        raise ValueError("Running subset cross-validation requires at least 2 subset groups.")
    if test_indices.size == 0:
    ...
    logging.info("="*120)
    logging.info("Subset labels: %s", subset_labels)
    logging.info("Test indices: %s", test_indices)
    ...
    for fold_idx, val_label in enumerate(subset_labels, start=1):
        ...
        logging.info("\n" + "-"*120)
        logging.info("[Fold %d/%d] Validation set: %s", fold_idx, len(subset_labels), val_label)
        logging.info("-"*120)
        ...
        logging.info("\n✓ Fold %d 训练完成 [最佳epoch=%d]", fold_idx, best_epoch)
    ...
    # Use best fold hyperparameters configuration, retrain final model using all non-test set data
    logging.info("\n" + "="*120)
    logging.info("Use best fold hyperparameters configuration, retrain final model using all non-test set data")
    logging.info("="*120)
    
    # Get all non-test set data (subset1+subset2+subset3)
    all_subset_indices = np.concatenate([subset_indices_map[label] for label in subset_labels])
    
    # Divide training set and validation set from all non-test set data (for final model training)
    ...
    logging.info("  Training set: %d samples (part of all non-test set data)", len(train_indices_final))
    ...
    logging.info("\nStart training final model (using all non-test set data)...")
    ...
    # Calculate final model training set metrics (using all non-test set data)
    device = torch.device(base_config.device)
    ...
    # Training set metrics (all non-test set data)
    ...
    logging.info("\n" + "="*120)
    logging.info("Final model performance (using best fold hyperparameters, training with all non-test set data)")
    logging.info("="*120)
...
def export_cv_metrics_to_excel(
    ...
    with pd.ExcelWriter(save_path, engine="openpyxl") as writer:
        ...
        if cv_results[0]["test_metrics"]:
            avg_data["Metric category"].append("Test set average PINN")
        ...
        if cv_results[0].get("test_xiao_metrics"):
            avg_data["Metric category"].append("Test set average Xiao")
        ...
        if "full_dataset" in best_model_results:
            best_model_data.append({
                ...
                "模型": "PINN",
                "样本数": best_model_results["full_dataset"]["sample_count"],
        ...
        # Calculate three-fold average training set metrics
        ...
        # Calculate three-fold average test set metrics
        if cv_results[0]["test_metrics"]:
...
def save_artifacts(
    ...
    # 3. Save data preprocessors (needed for subsequent inference with the same scaler)
    import pickle
    scaler_data = {
...
def print_training_config_summary(train_config: TrainingConfig) -> None:
    ...
    logging.info("  - Hidden layer number: %d", train_config.num_layers)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--tune-trials", type=int, default=50, help="Optuna search trials (recommended 50+ to achieve 0.87 performance)")
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    if not args.skip_tuning:
        ...
        base_train_config.max_epochs = args.final_epochs
        base_train_config.patience = max(20, base_train_config.max_epochs // 5)
        logging.info("\n✓ Best hyperparameters applied to final training configuration")
    else:
    ...
    # ==================== Cross-validation based on predefined Subset ====================
    logging.info("\n" + "#"*80)
    ...
    try:
        ...
        # Get all data features (already standardized)
        ...
        if 'No_Customized' in df_original.columns:
            ...
            logging.warning("No_Customized column not found, will match prediction results in row order")
            # If there is no No_Customized column, match prediction results in row order (ensure order consistency)
    ...
    logging.info("  2. View dataset_with_PINN_peak_strain.xlsx to view prediction values for all samples")
    logging.info("  3. Run inference script to evaluate model and compare with Xiao formula")
