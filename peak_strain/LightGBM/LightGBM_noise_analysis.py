...
# -*- coding: utf-8 -*-
"""
LightGBM Model Noise & Uncertainty Analysis

Functions:
1. Load pre-trained LightGBM model
2. Noise robustness: Test robustness to Gaussian noise and outliers
3. Prediction interval: Generate confidence intervals via Bootstrap & Quantile Regression

Note: Run LightGBM_train.py first to train the model, then load the saved model for analysis
"""
...
import numpy as np
import pandas as pd
	# Set matplotlib backend to Agg (non-interactive, avoid GUI errors)
import matplotlib
...
def configure_plot_fonts(fonts=None):
    """Unify Matplotlib fonts: ensure negative signs display correctly"""
    if fonts is None:
...
plt.rcParams['mathtext.default'] = 'regular'  # Render math symbols with regular fonts

# Working Directory Configuration - Automatically Locate the Project Root Directory
def find_project_root():
    """Locate the project root directory"""
    # First attempt to locate from the script's position
    try:
...
def load_data():
    ...
    # Extra features: peak stress
    extra_features = ['fc']
    # Formula features: Xiao formula
    formula_features = ['Xiao_strain']
    ...
    if missing_cols:
        ...
        # 如果缺少Xiao_strain，尝试移除（如果需要可以后续计算）
        if "Xiao_strain" in missing_cols:
            print("  Trying to remove Xiao_strain...")
            feature_names = [f for f in feature_names if f != "Xiao_strain"]
            print(f"  Removed Xiao_strain, current feature count: {len(feature_names)}")
        else:
    ...
    print(f"Target variable '{target_column}' range: {np.min(y):.6f} - {np.max(y):.6f}")
    
    # Save original DataFrame indices (for mapping predicted results later)
...
def split_data_by_divisions(X, y, sample_divisions, sample_ids, test_ratio=0.2, val_ratio=0.2, random_state=42):
    ...
    if train_indices and not val_indices:
        print("Only training set samples, splitting validation set from training set...")
        from sklearn.model_selection import train_test_split
        
        train_indices = np.array(train_indices)
        
        if len(train_indices) > 1:
            # If test set exists, only split the validation set from the training set
            # If test set does not exist, split the validation set and test set from the training set
...
def objective(trial, X_train, y_train, X_val, y_val):
    """Optuna optimization objective function - use validation set for evaluation (LightGBM version)"""
...
def optimize_lightgbm(X_train, y_train, X_val, y_val, n_trials=100):
    """Use Optuna to optimize LightGBM hyperparameters - use 5-fold cross-validation for evaluation"""
    print(f"\n=== Start LightGBM hyperparameter optimization (n_trials={n_trials}) ===")
    print("Use 5-fold cross-validation for hyperparameter evaluation, improve stability")
...
def train_lightgbm(X_train, y_train, X_val, y_val, best_params):
    """Use best parameters to train the final LightGBM model"""
...
def plot_train_test_comparison(model, X_train, y_train, X_test, y_test, save_dir):
    """Plot the comparison between training set and test set (with edge distribution) - optimized version
    
    返回:
        DataFrame: 包含训练集和测试集预测结果的DataFrame
    """
    print("\n=== Plot the comparison between training set and test set ===")
    ...
    print(f"\nTraining set performance - R2: {r2_train:.4f}, MAE: {mae_train:.6f}, RMSE: {rmse_train:.6f}")
    print(f"Test set performance - R2: {r2_test:.4f}, MAE: {mae_test:.6f}, RMSE: {rmse_test:.6f}")
    print(f"Performance difference - ΔR2: {abs(r2_train - r2_test):.4f}, ΔMAE: {abs(mae_train - mae_test):.6f}")
...
def plot_feature_importance(model, feature_names, save_dir, X=None):
    ...
    # Here is not saved directly, return DataFrame for main function to save
...
def plot_shap_analysis(model, X_background, X_explain, feature_names, save_dir):
    ...
    if is_ensemble:
        ...
        for i, sub_model in enumerate(model.models):
            ...
            try:
                ...
                print(f"  ⚠ SHAP calculation failed for sub-model {i+1}: {e}")
                ...
                try:
                    ...
                    print(f"  ⚠ All SHAP methods failed for sub-model {i+1}: {e2}")
        ...
        if len(shap_values_list) > 0:
            ...
            print("✓ Ensemble model SHAP values calculation completed")
        else:
    ...
    # Get current axes
    ax = plt.gca()
    # Do not change data range, only lengthen the physical ratio of the horizontal axis by increasing the graphic width
    plt.tight_layout()
    ...
    print(f"✓ Generated {n_samples_waterfall} samples' Waterfall plot")
    ...
    if len(low_importance_features) > 0:
        ...
        print("\nPossible reasons:")
        print("  1. Feature value change range is small (variance is small), resulting in limited impact on prediction")
        print("  2. Highly correlated with other features, replaced by other features")
        print("  3. Feature values are evenly distributed in the data set, lacking distinction")
        print("  4. This feature确实对目标变量影响较小（符合物理/工程规律）")
        print("  5. Sample size is small (94 samples), the impact of some features may be masked by noise")
        print("\nSuggestions:")
...
def analyze_feature_statistics(X, y, feature_names, shap_importance, save_dir):
    ...
    # shap_importance is already sorted in descending order of importance
    feature_stats_list = []
    ...
    for idx, row in shap_importance.iterrows():
        ...
        # Calculate the statistics of this feature
    ...
    # feature_stats is already sorted in the order of shap_importance (consistent with SHAP summary plot)
    
    # Save statistics (will be saved in the main function)
    # Return DataFrame for main function to save
    ...
    if len(low_importance) > 0:
        ...
        for idx, row in low_importance.iterrows():
            ...
            if row['cv'] < 0.1:
                reasons.append("Small CV (data change range is small)")
    ...
    # Analyze features with high impact
    high_importance = feature_stats.head(5)
    print(f"\nTop 5 features with high impact:")
    ...
    ax3 = axes[1, 0]
    # feature_stats is already sorted in descending order of SHAP importance, take the top 10
    # Note: SHAP summary plot is sorted from top to bottom of importance, so the most important is at the top
    top_features = feature_stats.head(10).copy()
    
    # Check and print detailed information of the w/c feature
    ...
    if len(wc_feature) > 0:
        ...
        if wc_row['range'] < 1e-6:
            ...
            print(f"  This will cause SHAP importance to be 0, because the feature value cannot affect the prediction.")
    ...
    if len(valid_range_features) < len(top_features):
        ...
        print(f"  All features will be plotted (including range 0), to find the problem")
    
    y_pos = np.arange(len(top_features))
    # Plot horizontal bar chart, the most important feature is at the top (index 0)
    ...
    # Invert the y-axis, so the most important feature is at the top (consistent with SHAP summary plot: importance decreases from top to bottom)
    ax3.invert_yaxis()
    ...
    ax4.set_title('Correlation with the target variable (absolute value top 10)', fontsize=14, fontweight='bold')
...
def plot_pdp_analysis(model, X_train, feature_names, save_dir, n_top_features=5):
    ...
    n_rows = (n_top_features + 2) // 3  # 调整行数计算
    ...
    for idx, (feature, ax) in enumerate(zip(top_features, axes)):
        try:
            ...
            # Calculate 95% confidence interval of ICE curves
    ...
    print(f"Single variable PDP analysis completed, plotted the dependencies of the top {n_top_features} important features")
    ...
    if n_top_features >= 2:
        ...
        print(f"Double variable PDP analysis completed, plotted the interactions between the top {n_interaction_features} important features")
...
def analyze_noise_robustness(model, X_test, y_test, feature_names, save_dir):
    ...
    # 3. Plot robustness analysis figure
    ...
    print(f"   - 0% outlier when RMSE: {outlier_rmses[0]:.3f} MPa")
    print(f"   - 2% outlier when RMSE: {outlier_rmses[1]:.3f} MPa (change {outlier_rmses[1]-outlier_rmses[0]:.3f} MPa)")
    print(f"   - 4% outlier when RMSE: {outlier_rmses[2]:.3f} MPa (change {outlier_rmses[2]-outlier_rmses[0]:.3f} MPa)")
    print(f"   - 6% outlier when RMSE: {outlier_rmses[3]:.3f} MPa (change {outlier_rmses[3]-outlier_rmses[0]:.3f} MPa)")
    print(f"   - 8% outlier when RMSE: {outlier_rmses[4]:.3f} MPa (change {outlier_rmses[4]-outlier_rmses[0]:.3f} MPa)")
    print(f"   - 10% outlier when RMSE: {outlier_rmses[5]:.3f} MPa (change {outlier_rmses[5]-outlier_rmses[0]:.3f} MPa)")
...
def analyze_prediction_intervals(model, X_test, y_test, save_dir, n_bootstrap=100, bootstrap_noise_level=0.01):
    ...
    # 3. Plot the comparison of prediction intervals
...
def train_single_fold(X_train, y_train, X_val, y_val, X_test, y_test, train_ids, val_ids, test_ids, 
    ...
    """Single fold training (for cross-validation) - only for hyperparameter optimization and training, no interpretability analysis"""
...
# Ensemble model wrapper (for handling saved ensemble models)
class EnsembleModelWrapper:
    """Ensemble model wrapper, for handling ensemble models loaded from files"""
    ...
    def feature_importances_(self):
        ...
        # Weighted average the feature importance of all models
        for model, weight in zip(self.models, self.weights):
...
def load_trained_model(model_path):
    ...
    if isinstance(loaded_data, dict):
        ...
        if loaded_data.get('type') == 'ensemble':
            ...
            # Normal dictionary format, try to extract the model and feature names
    ...
    if 'feature_names' in model_data and model_data['feature_names'] is not None:
        ...
        print("⚠ Warning: Feature names not found, using the feature names when loading the data")
...
def cross_subset_validation(X, y, feature_names, sample_divisions, sample_ids, original_df_indices, df_processed, 
    ...
    for i, val_label in enumerate(subset_labels):
        ...
        # Train a single fold model (only for hyperparameter optimization and training, no interpretability analysis)
    ...
    print(f"\n{'='*60}")
    print("Use the best fold model as the final model (no further training)")
    print(f"{'='*60}")
    ...
    print(f"The final model comes from the fold {best_fold_result['val_label']} of the validation set.")
    ...
    try:
        ...
        # Create a dictionary of prediction results
        pred_dict = {}
        ...
        print(f"  - New column: LightGBM_fc (final model prediction values)")
...
def cross_random_kfold_validation(X, y, feature_names, sample_ids, sample_divisions=None,
    ...
    print(f"The final model is trained using Fold {best_fold_result['fold_idx']}.")
    print(f"  R2: {final_r2:.4f}")
    print(f"  MAE: {final_mae:.3f} MPa")
    print(f"  RMSE: {final_rmse:.3f} MPa")
    if avg_r2 is not None:
    ...
    print(f"\n{'='*60}")
    print("Start interpretability analysis (random division)")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("Predict all samples and write to Excel (random division model)")
    print(f"{'='*60}")
    try:
        X_all_samples = X
        y_all_pred = final_model.predict(X_all_samples)
        ...
        df_original.to_excel(output_file, index=False)
        print(f"✓ Random division model prediction results written to: {output_file}")
        ...
        print(f"Warning: Error writing to random division Excel: {e}")
        import traceback
        traceback.print_exc()
...
def main():
    ...
    print("=== LightGBM SHAP analysis, noise analysis and uncertainty analysis ===")
    ...
    # 2. Load the trained model - directly using lightgbm_final_model.joblib
    ...
    # If the model data has feature names, use the model's feature names (ensure feature order is consistent)
    if 'feature_names' in model_data and model_data['feature_names'] is not None:
        model_feature_names = model_data['feature_names']
        print(f"\nUse the model's feature names ({len(model_feature_names)} features)")
        ...
        if len(model_feature_names) == len(feature_names):
            ...
            if set(model_feature_names) == set(feature_names):
                ...
                print("⚠ Warning: Model feature names do not match the data feature names")
    ...
    if noise_rmses is not None:
        ...
        print(f"  - No noise时RMSE: {noise_rmses[0]:.3f} MPa")
        print(f"  - 10% noise时RMSE: {noise_rmses[-1]:.3f} MPa")
    ...
    print(f"\n{'='*60}")
    print("Save all analysis results to an Excel file")
    print(f"{'='*60}")
    try:
        ...
        # Only save if at least one sheet needs to be saved
        if len(sheets_to_save) > 0:
            ...
            print(f"  - Total saved {len(sheets_to_save)} sheets")
            # Count the number of sheets
            try:
                import openpyxl
                wb = openpyxl.load_workbook(excel_path)
                sheet_count = len(wb.sheetnames)
                print(f"  - Sheet list: {', '.join(wb.sheetnames)}")
            except:
                pass
        else:
